...
def csv_stat(lines_csv, group, stat, command) :
    ...
    for row_list in lines_csv[1:] :        
        if command == "max" :
            try :
                result_dict[row_list[group_index]] = max(result_dict[row_list[group_index]],r_float(row_list[stat_index]))
            except :
                result_dict[row_list[group_index]] = r_float(row_list[stat_index])
        elif command == "min" :
            try :
                result_dict[row_list[group_index]] = min(result_dict[row_list[group_index]],r_float(row_list[stat_index]))
            except :
                result_dict[row_list[group_index]] = r_float(row_list[stat_index])
        elif command == "count" :
            try :
